...
public class BowlingFunctionSetDialog extends BowlingCommonDialog implements View.OnClickListener {
    ...
    // 设置TvMode方法,有则显示为白色,无则显示为灰色
    private void setTvMode(TextView tvTvMode){
    ...
    // 设置更新时间,包含广告更新时间和动画更新时间
    private void setUpdateTime(TextView tvAdTime,TextView tvAnimationTime){
        String adUpdateTime = (String) SharedPreferencesUtils.getParam(SharedPreferencesUtils.AD_UPDATEIME, "");
        String animationUpdateTime = (String) SharedPreferencesUtils.getParam(SharedPreferencesUtils.ANIMATION_UPDATETIME, "");
        if (!TextUtils.isEmpty(adUpdateTime)) {
            tvAdTime.setText(String.format(Locale.getDefault(),"广告_%s",adUpdateTime));
        }
        if (!TextUtils.isEmpty(animationUpdateTime)) {
            tvAnimationTime.setText(String.format(Locale.getDefault(),"动画_%s",animationUpdateTime));
        }
    }
    ...
    private void setLinearLayoutVisibleOrGone(int id) {
        ...
        if (id == R.id.ll_channel_container && linearLayoutChannel == null) {
            ...
            // 设置tvMode
            setTvMode(tvTvMode);
            final TextView tvAdUpdateTime = (TextView) findViewById(R.id.tv_ad_update_time);
            final TextView tvAnimationTime = (TextView) findViewById(R.id.tv_animation_update_time);
            // 设置更新时间
            setUpdateTime(tvAdUpdateTime,tvAnimationTime);
